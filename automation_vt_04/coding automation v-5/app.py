...
def api_check():
    ...
    if not ioc_value: return jsonify({"error": "IoC value cannot be empty"}), 400

    if ioc_type == 'hash':
        results_dict = run_hash_checks(ioc_value)
    else:
        # Handle non-hash types if needed
        results_dict = {}

    formatted_string = format_results_to_string(results_dict, ioc_value, file_name)
    
    return jsonify({
        "ioc_value": ioc_value,
        "file_name": file_name,
        "scan_output": formatted_string
    })
